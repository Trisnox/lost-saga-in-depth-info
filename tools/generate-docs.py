...
if __name__ == '__main__':
    ...
    with open('evolution_mercenaries.txt', 'r') as f:
        valid_evolutions = f.readlines()
        valid_evolutions = [_.strip() for _ in valid_evolutions]
...
/>"""
    ...
    for x in valid_evolutions:
        image1 = valid_images.get(f"thum_char_view_n_{x[:3]}.jpg", '')
        image2 = valid_images.get(f"thum_char_view_o_{x[:3]}.jpg", '')
        if not image1 == '':
            image1 = f"<Image img={{require('{image1}')}}/>"
        if not image2 == '':
            image2 = f"<Image img={{require('{image2}')}}/>"
